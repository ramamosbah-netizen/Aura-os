...
- **2026-08-01 update — serial/batch tracking added** (the thinnest domain's biggest ELV gap). New `SerialUnit` aggregate + `/inventory/serials` UI: register a serialised unit (unique per tenant+item), then track it in_stock → issued (to project) → installed (with warranty clock) → returned/faulty, with a status filter. Migration 0199 (RLS + unique-serial index). Verified E2E incl. the install-only-from-issued (409) and duplicate-serial (400) guards.
- **2026-08-01 update — warehouse/bin locations added.** New `StorageLocation` master (mig 0200, unique bin-per-warehouse) + `/inventory/locations` UI: register warehouses/stores/vans and their bins (bin/rack/shelf/floor/yard/van), grouped by warehouse, activate/deactivate. Verified E2E (create → duplicate-bin 400 → deactivate). **Still missing:** barcode/mobile picking, cycle counting.
