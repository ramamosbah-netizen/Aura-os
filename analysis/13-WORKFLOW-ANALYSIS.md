...
| 11 | **Commissioning** | 🟡 *(2026-08-01: built)* | New `commissioning` module + `/commissioning` UI: register each ELV system, record its test-point pass rate, commission it with a witnessed sign-off (state machine `pending→in_progress→tested→commissioned`, guard blocks sign-off until all points pass). Verified E2E against the live DB. **2026-08-01: when the LAST system on a project is commissioned, a draft handover package is auto-opened** (`commissioning.record.commissioned` → reactor checks all-commissioned + no existing package → `HandoverService.create`), so stage 11 now triggers stage 12 automatically |
| 12 | **Handover** | 🟡 *(2026-08-01: built)* | New Handover aggregate + `/handover` UI: per-project acceptance package with a close-out checklist (O&M manuals, as-builts, test certs, warranty, training, spares), submit→client accept→**warranty clock starts**, guarded state machine, and a **live link to stage 11** (each package shows how many of its project's systems are commissioned). Verified E2E. **2026-08-01: handover acceptance now auto-opens an AMC service contract** (`handover.accepted` event → app-layer reactor → `AmcService.createContract`, warranty window from the package, idempotent per handover) — closing deliver→maintain. Remaining: auto-generate the O&M/as-built document bundle from DMS |
| 13 | **Maintenance (AMC)** | 🟡 | AMC contracts + PPM schedules + WO costing (migs 0038/0078/0083); **field execution loop missing** |
| 14 | **Service** | 🟡 *(2026-08-01: built)* | New `/amc/dispatch` field-service board over the existing AMC backend: raise WO → **assign technician** → **complete with billable cost** (→ AMC/AR invoice), technician filter. Verified E2E. Remaining: technician mobile/PWA with offline + on-site checklist/photos + customer e-signature |
...
3. **Commissioning → handover → activate warranty → start AMC** — the ELV lifecycle chain. *2026-08-01: FULLY AUTOMATED end-to-end — all systems commissioned (11) → auto-opens a draft handover (12) → client acceptance auto-starts the AMC contract (13). All three links verified E2E through the event spine.*
