...
export function classifyDomainMessage(m: string): DomainClassification {
  ...
  if (
    /\balready\b|\blineage\b.*\bwithout\b|is closed|is inactive|is not (in|active|approved)|\bonly\b.*\bcan\b|can only\b|requires approval|below the required|insufficient|outside its validity|belongs to another/i.test(m)
  ) {
