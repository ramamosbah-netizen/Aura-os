...
export class PostgresMailStore implements MailStore {
  ...
  async findByInternetMessageId(tenantId: string, internetMessageId: string): Promise<MailRecord | null> {
    // Scoped to the TENANT, and deliberately not narrower: a reply may arrive on a different
    // mailbox than the one the original went out through and still belong to the conversation, so
    // account is the wrong boundary — but tenant is not optional.
    //
    // A Message-ID is generated by whoever sent it, so it is neither unique nor trustworthy across
    // organisations: a re-used or spoofed one must never let a message in one tenant graft itself
    // onto another tenant's thread. The predicate below is what prevents that. RLS would also
    // refuse it, but a domain invariant must not depend on the policy happening to be engaged —
    // the store is queried from workers and migrations too.
    const { rows } = await this.pool.query<MailRow>(
      `select * from public.aura_comms_mail
