...
export class MeetingController {
  constructor(private readonly meetings: MeetingService, private readonly tenant: TenantContext) {}
  private caller() { const ctx = this.tenant.get(); return { tenantId: ctx.tenantId, companyId: ctx.companyId ?? null, userId: ctx.actorId ?? DEV_USER }; }

  @Get()
  list(@Query('scope') scope?: string) { const c = this.caller(); return this.meetings.list(c.tenantId, c.companyId, scope); }

  @Get(':id')
  get(@Param('id') id: string) { return this.meetings.get(this.caller().tenantId, id); }

  @Post()
  @Permissions('comms.channel.send')
  create(@Body() dto: { title?: string; meetingType?: string; startsAt?: string; endsAt?: string; timezone?: string; location?: string; onlineUrl?: string; attendees?: Array<{ userId?: string; address?: string; displayName?: string; response?: 'pending' | 'accepted' | 'declined' | 'tentative' }>; relatedType?: string; relatedId?: string; relatedName?: string; agenda?: string }) {
    const c = this.caller();
  ...
  @Patch(':id')
  @Permissions('comms.channel.send')
  update(@Param('id') id: string, @Body() dto: MeetingPatch) { const c = this.caller(); if (dto.meetingType && !MEETING_TYPES.includes(dto.meetingType)) throw new BadRequestException('Unknown meeting type'); return this.meetings.update(c.tenantId, id, dto); }

  @Post(':id/items')
  @Permissions('comms.channel.send')
  addItem(@Param('id') id: string, @Body() dto: { kind?: string; title?: string; detail?: string; ownerId?: string; dueAt?: string }) { const c = this.caller(); return this.meetings.addItem(c.tenantId, c.userId, id, dto); }

  @Patch(':id/items/:itemId')
  @Permissions('comms.channel.send')
  updateItem(@Param('id') id: string, @Param('itemId') itemId: string, @Body() dto: { status?: 'open' | 'done' | 'cancelled' }) { return this.meetings.updateItem(this.caller().tenantId, id, itemId, dto); }

  @Post(':id/close')
  @Permissions('comms.channel.send')
  close(@Param('id') id: string, @Body() dto: { minutes?: string }) { const c = this.caller(); return this.meetings.close(c.tenantId, id, dto?.minutes ?? null); }
}
