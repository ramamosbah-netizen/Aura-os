...
function fixture() {
  ...
  return {
    service: new ScheduleResourceCatalogService(hr as never, fleet as never, assets as never),
    hr,
  };
}
...
describe('ScheduleResourceCatalogService', () => {
  ...
  it('accepts persisted tenant resources and rejects invented ids and unconnected pools', async () => {
    const { service } = fixture();
    const task = (resourceType: 'employee' | 'pool', canonicalResourceId: string) => [{
      name: 'Install', plannedStart: '2026-09-20', plannedEnd: '2026-09-21',
      requirements: [{ resource: { resourceType, canonicalResourceId }, quantity: 1, unit: 'persons' as const }],
    }];
    ...
    await expect(service.assertCanonicalReferences('tenant-1', task('pool', 'pool-1'))).rejects.toThrow('governed pool register');
  });
});
