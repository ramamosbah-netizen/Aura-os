...
describe('G-32 engineering drawing workflow (HTTP)', () => {
  ...
  it('enforces the lifecycle and records every transition end-to-end', async () => {
    ...
    await http.post(`${B}/drawings/${id1}/transmit`).send({ purpose: 'For Information' }).expect(400);
    await http.post(`${B}/drawings/${id1}/transmit`).send({ recipient: 'Consultant', purpose: 'For Information' }).expect(201);
    ...
    expect(conveyed).toMatchObject({ status: 'sent', recipient: 'Consultant', purpose: 'For Information' });
