...
export async function GET(_r: Request, { params }: { params: Promise<{ id: string }> }): Promise<Response> {
  ...
  try {
    const res = await apiFetch(`${apiBase()}/api/v1/crm/opportunities/${id}/scopes`, { headers: await authHeader(), cache: 'no-store' });
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }): Promise<Response> {
  const { id } = await params;
  try {
    const res = await apiFetch(`${apiBase()}/api/v1/crm/opportunities/${id}/scopes`, {
