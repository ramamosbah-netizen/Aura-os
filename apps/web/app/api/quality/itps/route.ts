...
export async function GET(request: Request): Promise<Response> {
  ...
  try {
    const res = await apiFetch(`${apiBase()}/api/v1/quality/itps${suffix}`, { headers: await authHeader(), cache: 'no-store' });
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    const res = await apiFetch(`${apiBase()}/api/v1/quality/itps`, {
      method: 'POST',
