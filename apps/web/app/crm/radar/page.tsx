...
export default async function RadarPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const result = await fetchJson<RadarData>(`/api/crm/signals/radar${query.toString() ? `?${query}` : ''}`);
  return (
    <section aria-labelledby="sales-radar-title">
      ...
      {!result.ok ? <DataStateNotice error={result.error} subject="Sales Radar" /> : <SignalsRadar data={result.data} initialQuery={Object.fromEntries(query.entries())} />}
    </section>
  );
}
