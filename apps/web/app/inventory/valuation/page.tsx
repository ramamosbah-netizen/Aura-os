...
interface Item { id: string; code: string; name: string; warehouse: string; unit: string; quantityOnHand: number; avgCost: number; stockValue: number }
interface Valuation { items: Item[]; totalValue: number }

function n(v: number, d = 2) { return v.toLocaleString(undefined, { minimumFractionDigits: d, maximumFractionDigits: d }); }

export default async function ValuationPage() {
  const v = await getJson<Valuation>('/api/inventory/stock/valuation');
  return (
    <div style={st.page}>
      <h1 style={st.h1}>Inventory · Valuation</h1>
      <p style={st.sub}>
        Stock value at moving-average (weighted-average) cost: value = on-hand × avg cost. Receipts
        update the average; issues are charged out (COGS) at the average.
      </p>
      {v === null ? (
        ...
        <div style={st.panel}>
          <table style={st.table}>
            ...
            <tbody>
              {v.items.map((i) => (
                <tr key={i.id}>
                  <td style={st.td}><span style={st.code}>{i.code}</span> {i.name}</td>
                  <td style={st.tdM}>{i.warehouse}</td>
                  <td style={st.tdNum}>{n(i.quantityOnHand, 0)} {i.unit}</td>
                  <td style={st.tdNum}>{n(i.avgCost, 4)}</td>
                  <td style={st.tdNum}>{n(i.stockValue)}</td>
                </tr>
              ))}
              <tr>
                <td style={st.totalLabel} colSpan={4}>Total inventory value</td>
                <td style={st.totalVal}>{n(v.totalValue)}</td>
              </tr>
            </tbody>
          </table>
        </div>
