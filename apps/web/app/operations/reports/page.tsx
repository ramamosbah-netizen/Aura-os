...
export default async function DeliveryOperationsReportsPage({
  ...
  return (
    <div style={{ padding: '28px 28px 64px' }} data-testid="delivery-operations-reports">
      ...
      <section className={styles.grid} aria-label="Delivery operations reports">{cards.map((card) => { const Icon = card.icon; return <Link key={card.label} href={card.href} className={styles.card} data-testid={`report-card-${card.label.toLowerCase().replace(/[^a-z0-9]+/g, '-')}`}><span className={styles.icon}><Icon size={17} aria-hidden /></span><span><small>{card.detail}</small><strong>{card.value}</strong><b>{card.label}</b></span><ArrowUpRight size={14} aria-hidden /></Link>; })}</section>
