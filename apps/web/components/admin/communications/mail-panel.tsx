...
/** SMTP mail relay configuration panel. */
export default function MailPanel() {
  return (
    <div style={st.panel}>
      <div style={st.header}>
        <h3 style={st.title}>📧 Outbound Mail & SMTP Relay</h3>
        <p style={st.desc}>
          SMTP relay endpoint configuration for outbound transactional emails (PO approvals,
          IPC certifications, SLA breach alerts). Tenant settings override environment defaults.
        </p>
      </div>

      <div style={st.card}>
        <div style={st.cardTop}>
          <h4 style={st.cardTitle}>SMTP Relay Status</h4>
          <Link href="/admin/notifications" style={st.link}>Configure Mail Relay →</Link>
        </div>
        <div style={st.ruleRow}>
          <span>SMTP Relay URL:</span>
          <span style={st.badgeGood}>Configured (Port 587)</span>
        </div>
        <div style={st.ruleRow}>
          <span>Fallback Recipient:</span>
          <span>admin@jeetintech.ae</span>
        </div>
        <div style={st.ruleRow}>
