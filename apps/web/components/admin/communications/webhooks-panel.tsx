...
export default function WebhooksPanel() {
  return (
    <div style={st.panel}>
      <div style={st.header}>
        <h3 style={st.title}>🔔 Outbound Webhooks</h3>
        <p style={st.desc}>
          Configure outbound webhook subscriptions for real-time event delivery to external systems.
          Each webhook can subscribe to specific domain events with retry and backoff policies.
        </p>
      </div>

      <div style={st.card}>
        <div style={st.cardTop}>
          <h4 style={st.cardTitle}>Webhook Subscriptions</h4>
          <Link href="/admin/webhooks" style={st.link}>Manage Webhooks →</Link>
        </div>
        <p style={st.cardDesc}>
          Configure endpoint URLs, event filters, authentication headers, and retry policies
