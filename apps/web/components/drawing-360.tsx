...
/**
 * Drawing 360 — the drawing, its revisions, its submissions and its reviews.
 *
 * It takes data that is already loaded. The ROUTE decides what may be loaded and checks that the
 * project in the path is the project the drawing actually belongs to; putting that fetch in here
 * would place the authorisation decision inside the thing being authorised.
 */
      ...
      <div style={st.crumbs}>
        {crumbs.map((c, i) => (
          <span key={c.label} style={{ display: 'contents' }}>
            {i > 0 && <span style={st.crumbSep}>/</span>}
            {c.href ? <a href={c.href} style={st.crumbLink}>{c.label}</a> : <span>{c.label}</span>}
          </span>
        ))}
      </div>
...
const st = {
  ...
  rowActive: { background: 'var(--info-soft)' } as CSSProperties,
