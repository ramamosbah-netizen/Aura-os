    ...
    <div>
      ...
      {activeTab === 'overview' && (
        <div>
                ...
                <ul style={st.attnList}>
                  {attention.map((a) => (
                    <li key={a.label} style={st.attnRow}>
                      <button onClick={() => selectTab(a.tab)} style={st.attnLink}>
          ...
          {/* The same shortcut grid Sales and the Delivery Operations overview use. Each card opens
              its section as an AURA tab beside the workspace's own, so a reader can keep RFIs and
              Submittals open at once instead of losing one to reach the other — which is all the
              top tab strip could ever do. */}
...
const st = {
  ...
  statPending: { fontSize: 11.5, color: 'var(--warn)', fontWeight: 600 } as CSSProperties,
  statClear: { fontSize: 11.5, color: 'var(--good)', fontWeight: 600 } as CSSProperties,
  ...
  attnCount: {
    ...
    border: '1px solid var(--warn-soft)', borderRadius: 6, padding: '2px 6px',
  } as CSSProperties,
  ...
  rfiQuestion: { fontSize: 13.5, margin: '0 0 10px', background: 'var(--panel-2)', padding: 10, borderRadius: 6, border: '1px solid var(--border)' } as CSSProperties,
  rfiAnswer: { fontSize: 13.5, margin: 0, background: 'var(--good-soft)', padding: 10, borderRadius: 6, border: '1px solid var(--good-soft)' } as CSSProperties,
