...
export default function HseControlClient({
  ...
  const { active: activeTab } = useWorkspaceSection<HseSection>(HSE_PATH, SECTION_IDS, 'incidents');
  ...
  return (
    <div>
      ...
      {/* The tab strip is gone: it was `HSE_SECTIONS` rendered twice, once here as buttons and again
              ...
              <table style={st.table}>
                ...
                <tbody>
                  {trainingRecords.map((r) => (
                    <tr key={r.id}>
                      <td style={{ ...st.td, fontWeight: 600, color: 'var(--text)' }}>{r.workerName}</td>
