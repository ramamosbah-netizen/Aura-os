...
function prettyDate(value: string | null): string {
  ...
  return Number.isNaN(parsed.getTime()) ? value : new Intl.DateTimeFormat(undefined, { day: 'numeric', month: 'short', year: 'numeric' }).format(parsed);
}
...
export default function MyTasksWorkspace({ initial }: { initial: WorkItemsPayload }) {
  ...
  const calendarTitle = calendarMode === 'month'
    ...
    : calendarMode === 'week'
      ? `${new Intl.DateTimeFormat(undefined, { day: 'numeric', month: 'short' }).format(agendaDays[0])} — ${new Intl.DateTimeFormat(undefined, { day: 'numeric', month: 'short', year: 'numeric' }).format(agendaDays[6])}`
      : new Intl.DateTimeFormat(undefined, { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' }).format(focusDate);
