    ...
    <main className={styles.page} data-testid="project-controls-dashboard">
      ...
      <section className={styles.controlGrid}>
        <div className={styles.matrixPanel}>
          ...
          <div className={styles.filters}><label className={styles.search}><Search size={15} /><span className={styles.srOnly}>Search projects</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Find a project, customer or contract" /></label><label className={styles.selectWrap}><SlidersHorizontal size={14} /><span className={styles.srOnly}>Filter status</span><select value={status} onChange={(event) => setStatus(event.target.value)}><option value="all">All statuses</option><option value="planned">Planned</option><option value="active">Active</option><option value="completed">Completed</option><option value="cancelled">Cancelled</option></select></label></div>
