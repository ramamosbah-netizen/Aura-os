...
export default function SiteControlClient({
  ...
  // Only `active` now: the strip that called `select` is gone, and the shortcut cards navigate by
  // href rather than through this hook.
  const { active: activeTab } = useWorkspaceSection<SiteSection>(SITE_PATH, SECTION_IDS, 'instructions');
  ...
  return (
    <div>
      ...
      {/* The tab strip is gone: it was `SITE_SECTIONS` rendered twice, once here as buttons and again
          as the shortcut cards at the foot of the page. The cards are the better half — each opens
          its section as an AURA tab beside the workspace's own, so two sections can be held open at
          once, where the strip could only swap one for the other.

          Nothing is stranded by removing it. That grid is rendered by the PAGE, after this client and
          outside any section condition, so it stays on screen whichever section is active — and the
          section itself is a real `?section=` address, so it survives a reload. */}
