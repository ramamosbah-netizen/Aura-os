      ...
      <section style={s.panelHeader}>
        <div style={s.headerMain}>
          <div>
            ...
            <p style={s.subtitle}>
              Customer: <strong>{tender.accountName ?? '—'}</strong> | Created: {new Date(tender.createdAt).toLocaleDateString('en-AE')}
    ...
    <section style={s.boqSection}>
      ...
      {latest ? (
        <div style={{ display: 'flex', gap: 16, alignItems: 'center', flexWrap: 'wrap', padding: '4px 0 8px' }}>
          ...
          <span style={{ fontSize: 12.5, color: 'var(--muted)' }}>decided {new Date(latest.createdAt).toLocaleDateString('en-AE')}</span>
            ...
            <div style={{ display: 'grid', gap: 6, marginTop: 10 }}>
              {records.slice(1).map((r) => (
                <div key={r.id} style={{ display: 'flex', gap: 12, alignItems: 'center', fontSize: 12.5, color: 'var(--muted)' }}>
                  ...
                  <span>{new Date(r.createdAt).toLocaleDateString('en-AE')}</span>
