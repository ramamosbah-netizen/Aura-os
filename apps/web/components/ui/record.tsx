...
export type { RelatedGroup, RelatedItem, ActivityEvent, LinkTone } from './related-records';

// ── AURA Record Design System (shared) ─────────────────────────────────────────
// Promoted out of components/crm/ (PR-04) so EVERY module — ELV, Engineering, Quality,
// HSE, Finance, not just CRM — composes the same record experience. crm/record-shell.tsx
// now re-exports this file, so existing CRM 360s are unchanged.
//
// One record experience for EVERY entity (Lead, Account, Device, NCR, Drawing, Invoice…).
// Every 360 page composes the same primitives so the whole platform reads as one system,
// not a pile of pages:
//
//   <RecordShell>
...
export function RecordShell({
  header, kpis, situation, tabs, activeTab, onTab, aside, children, footer, related, activity,
}: {
  header: ReactNode;
  kpis?: KpiItem[];
  situation?: ReactNode;
  ...
  children: ReactNode;
  footer?: ReactNode;
  /** Linked records web (connectivity) — rendered below the body when provided. */
  related?: { title?: string; groups: RelatedGroup[] };
    ...
    <div>
      ...
      {related && (
        <div style={{ marginTop: 16 }}>
          <RelatedRecords title={related.title} groups={related.groups} />
        </div>
      )}
      {activity && (
        <div style={{ marginTop: 16 }}>
          <RecordCard title={activity.title ?? 'Activity & history'}>
...
const rs: Record<string, CSSProperties> = {
  ...
  sGateChip: { fontSize: 12, border: '1px solid var(--warn)', borderRadius: 8, padding: '3px 10px', color: 'var(--warn)', background: 'color-mix(in srgb, var(--warn) 8%, transparent)' },
