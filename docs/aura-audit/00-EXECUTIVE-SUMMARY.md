...
- ~~**Back-half modules** (engineering, doc-control, quality, HSE, site, fleet, assets, AMC, commissioning) have real data models and stores but **thin orchestration and thin UI**.~~ **Superseded at Rev 2.** Engineering, doc-control, quality, site and commissioning are now **governed lifecycle engines**: each has an enforced state machine, child-record depth, a completable in-app journey, and browser E2E driving it (PRs #205–#209, migs `0224`–`0228`; evidence table in `02`). The claim now applies only to **HSE, fleet, assets and AMC**, which remain 1 controller / 1–3 pages of governed CRUD.
...
**Rev 2 (2026-08-12):** the maturity level and readiness score are **deliberately unchanged**. Five merged verticals closed the delivery-half functional gap (G-08 → P2) and cut the P1 count from 9 to 7 (G-08 downgraded to P2, G-07 closed outright), and the weighted component arithmetic rises 73.4 → 75.4 (`20`). But readiness is gated on three P0s, **all still open**, so the headline does not move. Two are ops actions; the third is now a scoped test suite with its harness already built. **The distance to a controlled pilot is shorter than at Rev 1, and it is no longer a product-completeness distance — it is a verification distance.**
