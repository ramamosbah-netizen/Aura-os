...
>
> **The poisoning claim is neither confirmed nor cleared.** It could not be reproduced at `1a14a036`, because `permit-workflow.spec.ts` does not exist there — it is on *this* branch only — and all five pairings available on `main` passed both before and after the fix. Playwright gives each test its own context, so the queue cannot leak by the route Rev 2.2 proposed; the one genuinely shared channel found was an **aborted in-flight request against the common `next dev` server**, which the redesign removes by letting the request finish server-side. Teardown of IndexedDB, the localStorage fallback, service workers and caches was added regardless. **Re-run `offline-sync` → `permit-workflow` on this branch once #210 lands to settle it.**
