...
| 2 | Auth ON by default + refresh/revocation + lockout | P0 | ◑ **mechanism done 2026-07-07, default NOT** (corrected 2026-08-05) — enforcement is opt-in via `AUTH_REQUIRED=true`, or automatic only when production **already has a verifier** (`main.ts:101`). Production without `AUTH_JWKS_URL`/`AUTH_JWT_SECRET` runs **open**. Verified live: `/auth/status` → `{"enabled":false}`, unauthenticated `GET /crm/opportunities` → 200 with 34 records. See [Vol 23 P0 row 2](vol-23-gap-analysis.md) and readiness audit P0-1. |
