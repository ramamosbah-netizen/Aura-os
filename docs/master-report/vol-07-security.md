...
| # | Item | Sev | Status |
|--:|---|---|---|
| 1 | RLS enforcement bundle (least-priv role + GUC + FORCE RLS + isolation test) | P0 | ◑ **built, not activated** (updated 2026-08-05) — least-priv `aura_app` role, tenant GUC binding, `FORCE RLS` on 182/182 tenant tables maintained by the CI `rls-fitness` gate, isolation test, **and a boot gate that refuses production under a `BYPASSRLS` role** all exist. The running instance still connects as the `postgres` owner, so **RLS is inert at runtime** (`⚠️ … RLS policies are INERT` logged at boot, verified live). Remaining work is the operator flipping `DATABASE_URL` to `aura_app` per [the runbook](../runbooks/rls-tenant-isolation.md). |
| 2 | Auth ON by default + refresh/revocation + lockout | P0 | ✅ **done 2026-07-07 — re-verified 2026-08-05.** `main.ts:52-63` refuses to boot in production when no verifier is configured: measured `NODE_ENV=production` → FATAL, **exit 1**, never listens (`ALLOW_INSECURE_NO_AUTH=true` is the deliberate escape hatch). *An earlier 2026-08-05 revision downgraded this row on a misreading and has been retracted — see [Vol 23 row 2](vol-23-gap-analysis.md).* The **dev default is auth off** by design (staged pass-through), which is why the local instance answers unauthenticated. |
