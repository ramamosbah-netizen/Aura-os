...
| **Form Designer** (§2.4, P1+P2) | `/admin/forms`. **P1**: per-field label/placeholder/hint/required/visible patches (`aura_form_overrides` 0136), merged by shared `applyFormOverrides` in the renderer **and** `assertFormValid` — designed = rendered = enforced. **P2 (2026-07-10)**: **add custom `cf_*` fields** (text/number/select/date/textarea, required, options) that render + validate + **persist per record** (`aura_form_custom_values` 0140; enforced endpoints validate the raw body and capture values; read at `GET /forms/:id/values/:recordId`) · **▲▼ reordering** (`order[]`) · **draft→publish cycle** (0139: designer edits a draft, Publish promotes atomically w/ version++, audited — the live form never sees half-finished designs). Verified live 9/9: draft isolation → publish v2 → missing custom field 400 w/ designed label → value captured + read back → reset. Remaining §2.4: layout/rules/formula editing; list-view/dashboard/menu designers = Vol 14 | ✅ 2026-07-10 (P2) |
...
| **Security posture** (depth wave, §2.2) | `/admin/security` — auth mode (JWKS/HS256/off), anonymous-request + dev-token/dev-password posture, lockout numbers, **MFA enrolments** (active/pending, `MfaService.listEnrolments`), SSO wiring + Entra group→role map, PII-crypto staging/rotation. Env-bound values read-only by design (runbook pointers). **+ API keys (§2.5, same day)**: service accounts (`aura_service_accounts` migration 0138, hash-only storage, key shown once) authenticate as `sa:<id>` via auth step 0 and authorize through normal role grants; create/revoke/reinstate/delete card w/ last-used; revocation immediate | ✅ 2026-07-10 |
...
| **Module Manager** (NEW-ERP parity) | `/admin/modules` — enable/disable the 17 business modules per tenant (kernel `ModulesService`, hydrate from settings key `modules.disabled` — no new table). **Enforced**: PermissionsGuard rejects a disabled module's routes (403, auth state irrelevant) AND the sidebar hides its items for every user (`GET /workspace/modules`). Data untouched; kernel surfaces never gateable; audited | ✅ 2026-07-11 |
| **Module Settings** (NEW-ERP parity) | `/admin/module-settings` — catalog-driven business defaults per module (`MODULE_SETTINGS_CATALOG` in shared): finance VAT/terms/fiscal-year, procurement thresholds, inventory low-stock/valuation, project stages/VO threshold, subcontract retention, HR leave/probation, CRM/AMC/fleet/assets defaults. Stored as tenant settings; **`subcontracts.defaultRetentionPercent` consumed live** (create default), the rest published for module adoption ("live" pill marks consumed keys) | ✅ 2026-07-11 |
...
| Remaining spec sections (§2.4 layout/rules/formula editing, §2.7 depth: prompt packs / cost meters, §2.8 digest schedules, list-view/dashboard/menu designers per Vol 14) | — | phase 2+ [Planned] |
