...
| 12 | Admin center phase 1 — **3 screens shipped 2026-07-07/08**: `/admin/access` Roles & Permissions (completes #7's roles UI), `/admin/webhooks` (subscriptions + delivery log), `/admin/approval-matrix` (ordered approval-rule editor). Established repeatable pattern (backend read/write → BFF → page → client → nav). Remaining: numbering / settings screens + PG-backed role store | Vol 15 §3 | S | every config change = engineering ticket |
