...
| # | Gap | Home | Effort |
|--:|---|---|---|
| 16 | Form designer — **P1 2026-07-09** (label/required/visibility overrides, 0136) · **P2 2026-07-10**: **add custom `cf_*` fields** (render + validate + values persist per record, `aura_form_custom_values` 0140, raw-body enforcement on all 3 endpoints), **reordering**, **versioned draft→publish** (0139, atomic promote + version++, audited; live form never sees drafts). Verified live 9/9 incl. draft isolation + designed-label 400 + value readback. · **P3 2026-07-11 — CLOSED**: designer-authored **business rules** (condition→action builder, appended after code rules), **formulas** (per-field, incl. computed `cf_*`; empty clears a code formula), **validation** (min/max/length/pattern, replaces code rules per field), **section layout** (replaces code layout, unplaced fields render after) — all merged by the same `applyFormOverrides`, so renderer + `assertFormValid` enforce them with zero new code paths; `validateFormOverrides` gates drafts server-side AND in-browser (broken formulas/refs/regex/rules/layout refused). Live 14/14 (designed rule → 400, designed validation → 400, publish/reset roundtrip); shared 175 tests. Remaining designers (list views/dashboards/menus) = Vol 14 / #23 | Vol 5 §10 | ✅ |
