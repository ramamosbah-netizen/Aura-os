...
| 8 | Global validation layer — **error half done 2026-07-06** (enforced error taxonomy: pure `classifyDomainMessage` + fitness test failing CI on any 500-escape, PR #31; audit: 57/389 domain throws escaped → 0). Remaining: server-side `evaluateForm` | Vol 9 §7 | S (was M) | metadata rules still bypassable via API |
