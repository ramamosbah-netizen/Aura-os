...
| 12 | Admin center phase 1 — **2 screens shipped 2026-07-07/08**: `/admin/access` Roles & Permissions (list/create roles, grant/revoke; completes #7's roles UI) and `/admin/webhooks` (register/enable/disable subscriptions + delivery log). Remaining: numbering / approval-matrix / settings screens + PG-backed role store | Vol 15 §3 | M–S | every config change = engineering ticket |
