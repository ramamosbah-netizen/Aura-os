...
- ⚠️ **Remaining:** the remaining (non-spine) L2/L3 modules still write via the inline access+`TX_RUNNER` path (equivalent atomic write), and the `Idempotency-Key` header is not yet enforced at the HTTP controllers (Part XV §2) — the key is plumbed through the bus but not required by handlers.
