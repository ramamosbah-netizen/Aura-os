...
- ✅ **Command pipeline exists and is used:** the entire **deal chain — CRM → Contracts → Projects — now creates through the `CommandBus`** (commits `289ada0`, `00da6f1`), each with a test exercising the real pipeline (persist + emit + validation; CRM also idempotent replay).
- ⚠️ **Remaining:** route the remaining modules' writes through the bus (the deal chain is the proven template; tendering/procurement/finance add numbering+audit inside the handler), and enforce the `Idempotency-Key` header at the controllers (Part XV §2) — currently the key is plumbed through the bus but not required by HTTP handlers.
