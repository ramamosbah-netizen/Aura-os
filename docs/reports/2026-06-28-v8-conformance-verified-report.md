...
- ✅ **Command pipeline exists and is used:** demonstrated end-to-end on the CRM create path with idempotency.
- ⚠️ **Remaining:** route the other 16 modules' writes through the bus (CRM is the template), and enforce the `Idempotency-Key` header at the controllers (Part XV §2) — currently the key is plumbed through the bus but not required by HTTP handlers.
