...
| 7 | Multi-tenant isolation via RLS | ⚠️ **Partial (better than first measured)** | *Re-audited 2026-06-29 — see addendum.* Of **71** distinct `aura_*` tables with RLS enabled, all but **2** now carry a tenant/hierarchical policy. The earlier "41 explicit policies" was a measurement artifact — it missed the **44 policies `0032` applies via a dynamic `DO $$` loop** (CREATE POLICY inside `format()` strings, invisible to a literal grep) plus whitespace-irregular literals. Remaining gaps: `aura_finance_bank_transactions` (fixed in `0052`) and the two kernel-infra tables `aura_events` / `aura_webhook_subscriptions` (RLS on, no policy → deny-all under a non-owner role; left for a DB-verified fix because a naive tenant policy could starve the cross-tenant outbox relay/dispatcher). |
...
- 🔴→⚠️ **pgvector RAG embeddings were fake** *(addressed 2026-06-29 — see addendum)*. `vector-store` calls `aiService.embed()`, which originally returned a **content-blind seed-filled pseudo-vector**. Now replaced with a deterministic **lexical (feature-hashing) embedding** so cosine reflects token overlap; a true *neural* embeddings provider remains the upgrade path for semantic (synonym/paraphrase) retrieval.
- ⚠️→✅ **Part IV CDM** *(completed 2026-06-29 — see addendum)*. The standard cites split files (`cdm/party.ts`, `…/location.ts`, etc.) that don't exist — everything lives in one `cdm.ts` — but the **types are now all present**: `Party`, `Address`, `Period`, `Quantity` (pre-existing) plus newly added `Location`, `Account`, `CostCenter`, `Project`. `Document` is the existing DMS `Document` (canonical, re-exported from the same package), deliberately not duplicated. Layout differs from the doc (one file vs many); the model coverage is complete.
...
### Embeddings finding resolved — lexical embeddings replace the seed-fill (commit `5ba8892`)
The original 🔴 finding was that `provider.embed()` returned a **content-blind PRNG seed-fill** — a 1536-vector seeded from the text but otherwise random, so pgvector cosine "semantic" search ranked **meaninglessly**. Both providers (Claude + local) now use a deterministic **lexical embedding** in `@aura/shared` (`lexicalEmbedding`): tokens feature-hashed into 1536 buckets (TF + sign hash), L2-normalized, so **cosine reflects shared-token overlap** — genuinely useful for lexical RAG. `lexical-embedding.test.ts` proves the property the seed-fill lacked (related text outranks unrelated; self-similarity ≈ 1; correct dim; deterministic). **Honest remaining gap (downgraded 🔴 → ⚠️):** this is *lexical*, not *neural* — no synonymy/paraphrase. True semantic retrieval still needs a dedicated embeddings provider (Voyage / OpenAI `text-embedding`); Anthropic exposes no first-party embeddings endpoint via the SDK, so that's the documented upgrade path.

### CDM completed — Part IV value objects all present (commit `f1279fe`)
The earlier ⚠️ was that the CDM held only `Party`/`Address`/`Period`/`Quantity` (~40% of Part IV). Added to `shared/src/domain/cdm.ts`: **`Location`** (+ `makeLocation` coordinate validation, plus `distanceMeters` haversine and `withinGeofence` helpers — directly useful for the AMC/Fleet/Site GIS dispatch the blueprint calls for), **`Account`** (+ `AccountType`), **`CostCenter`**, and **`Project` (CDM)** (+ `ProjectStatus`, the canonical execution context distinct from the Projects module aggregate). `Document` is intentionally the existing DMS `Document` (canonical, same package) rather than a duplicate. `cdm-geo.test.ts` covers validation, distance, geofence in/out, and the new shapes. The only residual deviation from Part IV is **layout** (one `cdm.ts` vs the doc's split files) — the type coverage is complete.

### Financial-correctness win — idempotent payment recording (commit `4ce802e`)
Beyond the spine creates, `PaymentService.record` was the highest-risk non-idempotent write: a multi-step operation (payment row → mark invoice paid → post the double-entry journal → emit) with no idempotency, so a retried-but-already-succeeded request posted a **second payment and a second ledger journal** — silent double-counting. It now dispatches through the `CommandBus` (flow preserved verbatim), the finance payments controller threads the `Idempotency-Key`, and `payment-idempotency.test.ts` proves with the **real** pipeline that a replayed key yields one payment + one journal (and distinct keys still record distinct payments). This is the kind of retry-safety the `Idempotency-Key` plumbing was built for, on the write where it matters most.

### Law #7 RLS — re-audited and one gap closed (commit, migration `0052`)
A precise static audit (matching both literal `CREATE POLICY` and the dynamic `DO $$`/`format()` policies) shows RLS coverage is **far better than the first pass measured**: of 71 RLS-enabled `aura_*` tables, only **3** lacked any policy — `aura_finance_bank_transactions`, `aura_events`, `aura_webhook_subscriptions`. A table with RLS *enabled but no policy* is **deny-all** under a non-owner role, so these only "work" today because the app connects as the table owner (which bypasses RLS) — i.e. RLS is currently latent, not enforced, on them.
- **Fixed:** `aura_finance_bank_transactions` — a plain business table simply omitted from `0032`'s list when it arrived later (`0046`). Migration `0052` attaches the identical tenant-only `tenant_isolation_policy` its sibling finance tables use.
- **Verified against the live Supabase DB (2026-06-29):** migrations are at **52/52** and `bank_transactions` now carries `tenant_isolation_policy`. The live catalog (`pg_policies`) shows **83 RLS-enabled `aura_*` tables, 78 with a policy, 5 without** — and all 5 are **kernel-infra**: `aura_events`, `aura_webhook_subscriptions`, `aura_webhook_deliveries`, `aura_documents`, `aura_document_versions`. (The live count exceeds the static one because several kernel tables enable RLS in ways the migration grep didn't capture.)
- **Reframed — the 5 are correct, not a gap.** These are internal tables that must never be reachable by client (anon/authenticated) roles, so *RLS-enabled with no policy = deny-all to clients*, which is the **right** posture. The application connects via the Supabase **service role**, which bypasses RLS, so its own access to events/webhooks/documents is unaffected. Net: the only genuine policy gap was the one business table (`bank_transactions`), now closed and verified live; Law #7's remaining nuance is defense-in-depth design (FORCE RLS / least-privilege app role), not missing policies.
