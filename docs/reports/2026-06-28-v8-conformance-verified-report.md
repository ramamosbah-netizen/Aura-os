...
- **Deferred (needs a live DB):** `aura_events` (the event store) and `aura_webhook_subscriptions` are read by **cross-tenant system processes** (the outbox relay, the webhook dispatcher) that don't run inside a tenant context — a naive `tenant_id = current_tenant_id()` policy would hide rows from those workers and break the spine. The correct fix (service-role bypass vs per-event tenant context) must be designed and verified against a running Postgres, so it was **not** changed blind.
