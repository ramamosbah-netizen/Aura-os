...
**Still open on Law #2:** writes are not yet routed through `CommandBus` — the command pipeline (`command.bus.ts`, idempotency, lock) remains reference-only, invoked by no controller or service. That (plus idempotency-key enforcement per Part XV §2) is the remaining half of the law.
