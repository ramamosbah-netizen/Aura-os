...
39. `[PENDING]` SWC transform for vitest → unblock Nest HTTP e2e.
40. `[PENDING]` Supertest E2E for the 4 core chains (P2P, O2C, deal-chain, service).
41. `[PENDING]` Playwright smoke for top 10 pages.
42. `[PENDING]` Coverage tooling + CI threshold gate.
43. `[PENDING]` Dependency/SAST scan (e.g. `npm audit`/Snyk) in CI.
44. `[DONE]` Global exception filter + error taxonomy.
45. `[DONE]` OpenAPI/Swagger generation + published spec.
46. `[DONE]` Renumber duplicate `0059` migration.
47. `[PENDING]` Add down-migrations (or adopt a migration tool with rollback).
48. `[DONE]` Standardize `date::text` mapping across all stores (kill TZ drift).
49. `[PENDING]` Type the 124 `any` pg-row mappers.
50. `[PENDING]` Root ESLint config + enforce in CI.
51. `[PENDING]` Decide & document FK policy (snapshot vs referential).
52. `[DONE]` Reporting **views** for hot read paths.
53. `[PENDING]` Bulk operations (create/update/delete) on high-volume modules.
54. `[DONE]` CSV/Excel import + export per module (CSV ✅, Excel ❌).
55. `[PENDING]` Soft-delete + restore standardized across modules.
56. `[PENDING]` Attachments/comments via DMS on all modules.
57. `[PENDING]` Idempotency-key required (not just honored) on spine creates.
58. `[PENDING]` Roll CommandBus to the ~10 non-spine modules (or ratify inline path).
...
**Session deltas (2026-07-01):** Finance multi-currency (AP+AR) + FX revaluation posting; standard pagination contract (COUNT + LIMIT/OFFSET + `Page` envelope) rolled out to Finance, Procurement, Inventory, CRM. **#22 remaining:** projects, contracts, tendering, engineering, subcontracts. **Blocked:** global `ValidationPipe` (#23) needs `class-validator` install.
