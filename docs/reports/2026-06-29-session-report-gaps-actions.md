...
| **CRM Quotations** | (this round) | `0065` | `POST/GET /crm/quotations`, `GET /:id`, `PATCH /:id/status` | pre-sales quote (deal-chain step before contract/invoice); net 11500 / VAT 575 / total 12075; draft → sent → accepted; expire-after-accept → 400; empty-lines → 400; JSONB lines + dates round-trip |
