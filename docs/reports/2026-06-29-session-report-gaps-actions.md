...
| **Finance Bank Guarantees** | (this round) | `0061` | `POST/GET /finance/bank-guarantees`, `GET /expiring`, `GET /:id`, `PATCH /:id/status` | create active (AED, expiry preserved) → release; double-transition → 400; expiry-before-issue → 400; bad type → 400; 15-day instrument appears on `expiring?withinDays=30` watch-list |
