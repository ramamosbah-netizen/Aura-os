...
> GitHub remote `origin` configured (`ramamosbah-netizen/Aura-os`); `main` pushed. ~55+ commits since baseline `cd08948`. Throughout: `pnpm typecheck` **42/42**, `pnpm test` **41/41** tasks (fleet 14 tests incl. 10 traffic-fine tests; HR **27 tests** incl. 9 expense-claim tests; finance now **42 tests** incl. 13 petty-cash + 11 customer-invoice tests; apps/api test runner wired this session), Supabase migrations **51 → 60** applied & verified live.
...
| **Finance Customer Invoices (AR)** | (this round) | `0060` | `POST/GET /finance/customer-invoices`, `GET /:id`, `POST /:id/{issue,receipts,cancel}` | net 12000 / VAT 600 / total 12600; draft → issued → partially_paid (6000) → paid (12600); overpay → 400; empty-lines → 400; JSONB line items + issue date survive round-trip |
