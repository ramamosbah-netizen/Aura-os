...
> ## ⛔ SUPERSEDED — historical snapshot, do not cite the verdict
>
> **Status as of 2026-08-05:** this document's "PRODUCTION READY" verdict is **not accurate for the current tree** and must not be quoted.
> It was written before the 2026-08-03 enterprise-readiness audit, which measured **54/100** on enterprise-production readiness and found blockers this report claims were closed:
>
> | This report claimed (2026-07-19) | Verified state (2026-08-05, live app) |
> |---|---|
> | "Every transaction is database-isolated under strict RLS" | RLS policies are **inert at runtime** — the app connects as the `postgres` owner role which bypasses RLS. The API now *warns* at boot in dev and *refuses to boot* in production, but the running configuration is still bypassed. |
> | "All core platform blockers (P0/P1) … fully resolved" | **P0-1 is open:** `GET /api/v1/crm/opportunities` with **no token** returns `200` with live records (auth is OFF by default). Re-verified live 2026-08-05. |
> | "PRODUCTION READY" | Pilot-grade. See the current audit for the live gap register. |
>
> **Read instead:** [2026-08-03-enterprise-readiness-audit.md](2026-08-03-enterprise-readiness-audit.md) (current gap register) and [2026-08-05-platform-state-verification.md](2026-08-05-platform-state-verification.md) (latest live verification).
> **Keep this file for:** the section 7/8/9 inventories (web pages, API handlers, domain events) and the July-19 gap-closure history.
...
**Verdict (as written, now superseded):** **PRODUCTION READY.** All core platform blockers (P0/P1) and business journey gaps are fully resolved, integrated, and verified via the automated CI deployment gate.
