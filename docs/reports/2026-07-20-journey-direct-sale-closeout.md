...
| D | **The UI does not refresh after its own action.** Observed five times: Opportunity → won, Quotation approve / send / accept, IPC submit. In each case the API had persisted the change while the screen still showed the old state and the old buttons. The user cannot tell whether their action worked without a manual reload. | 🟠 friction | API vs DOM compared at each step |
...
| User Guidance | **7/10** | AI read on the signal, inline win-reason, 100% on a won deal, and a button that names its own effect (*"Activate / Sign → creates Project"*). −3: the screen does not update after its own action (gap D), "Send to customer" sends nothing (gap E), and "EXACT MATCH" is asserted against three identical accounts (gap C). |
