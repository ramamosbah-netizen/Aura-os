...
- **P2-5 IA discoverability:** "Opportunities" not a nav word (under Pipeline→/crm/leads); orphan pages `/crm/commercial`, `/tendering/pricing`; ~5–7s first paint. **✅ NAV FIXED (2026-08-05):** the Sales sidebar now shows **"Pipeline & Opportunities"** (surfacing the term) and a new **"Commercial"** item linking the previously-unreachable `/crm/commercial` decision workspace (it had *zero* inbound links). `/tendering/pricing` was **not** actually orphan — it's reached from the Tenders page ("⊞ Pricing summary") and the Commercial workspace by design — so it was left as-is. Verified live. **Remaining:** ~5–7s first-paint latency (separate perf item).
