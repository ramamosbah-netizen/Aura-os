...
- **P2-4 AMC module inconsistency:** self-labelled "Asset Management & Contracts" vs "Annual Maintenance" elsewhere; different visual language. **✅ NAMING FIXED (2026-08-05):** the page H1 was the lone surface expanding AMC as "Asset Management & Contracts" — a made-up term that collides with the real Assets module; it now reads **"AMC & Services"** (matching the nav area/item, shared module registry, and domain), with the correct expansion — *Annual Maintenance Contracts* — folded into the subtitle. Verified live (breadcrumb, nav tab, page title all agree).
