...
**✅ FIX (2026-08-03):** Built the **shared UI kit** (`components/ui/kit.tsx`) — `Button` (primary/neutral/danger/ghost), `Field`, `Input`, `Select`, `Card`, `KpiTile`, `Badge`, `Table/Th/Td` — all on the **correct tokens** (`--text`/`--panel`/`--accent`/`--border`/`--good`/`--bad`/`--warn`), no `--fg`/`--surface`, no hardcoded hex. Migrated `daily-report-client` + `inspection-request-client` onto it: verified in the live DOM the primary button now renders `rgb(245,166,35)` = the amber `--accent` (was off-brand blue `#2563eb`), KPI/status colours use semantic tokens, and inputs use `--text`/`--panel` (fixes the invisible-input-text risk). **✅ TOKEN MIGRATION COMPLETE (2026-08-04):** the undefined-token bug is now fixed **app-wide** — a mechanical mapping across **52 files** replaced every `var(--fg)`→`var(--text)`, `var(--surface)`→`var(--panel)`, `var(--surface-2,…)`→`var(--panel-2)` (grep confirms 0 undefined tokens remain; web typecheck clean). So invisible/low-contrast input text is resolved everywhere, not just the 2 pilot screens. **✅ OFF-BRAND HEX PASS COMPLETE (2026-08-04):** the three off-brand hardcoded colours were mapped to semantic tokens app-wide — `#2563eb`→`--accent` (amber brand), `#16a34a`→`--good`, `#dc2626`→`--bad` (0 remain; dead `var(--accent, #2563eb)` fallbacks cleaned) — so status/KPI/link colours now theme in light+dark. The 24 solid semantic buttons (`background:#16a34a/#dc2626; color:#fff`) were tokenised **and** their white text moved to `var(--accent-ink)` (readable on `--good`/`--bad` in both themes — white failed WCAG on the dark-mode `--good`); the kit's own `danger` variant was converged onto the same. **Remaining (larger, follow-up): swap inline `<button>`/tables for the kit `<Button>`/`<Table>` components.**
...
**Findings:**
- **"Automatic after a manager/admin approves" is NOT how the commercial cycle works.** True threshold approval (auto-approve below a limit, else → `pending_approval`) exists **only in Procurement** (`ApprovalMatrixService`, PO/PR). **UPDATE (2026-08-03/04): both halves of the control now exist across quotation-approve / contract-sign / IPC-certify / invoice-approve** — (1) a **maker-checker** blocks self-authorisation (preparer → 403), and (2) a **value-threshold** check refuses an approver whose `approvalLimit` is below the record's value (→403 "above your approval limit"), reusing the access layer's ABAC (tiered demo approvers: manager ≤50k, director ≤500k, admin unlimited). Both skip auto/system flows. Verified in `sod.e2e-spec.ts`. Still: the steps only *unlock* the next — they don't auto-advance it.
