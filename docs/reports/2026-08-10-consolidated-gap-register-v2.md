...
| ID | Gap | Sev | Ev |
|---|---|:--:|:--:|
| ◑ **N-08** | **Accounts path CLOSED; sweep ratcheted.** Was: tenant B could list, read and mutate tenant A's account over HTTP. The `assertSameTenant`/`sameTenantOrNull` guard already existed but lived in `modules/finance/src/domain/` and was adopted by finance only — **promoted to `@aura/shared`** so every module can use it (finance keeps its import path via a re-export). CRM accounts now scopes `get`, `list`, `listPaged` and the fetch-before-mutate in `update`; a caller-supplied `tenantId` is overridden by the bound one rather than merged. **All 3 previously-failing assertions now pass: API e2e 34 files / 177 tests, no skips.** The remaining surface is **38 services** that fetch by id with no tenant check, pinned by a new fitness ratchet (`apps/api/src/tenant-isolation.fitness.test.ts`) that fails if the number rises *and* fails if it falls without being lowered — so the sweep can proceed incrementally and can only go one way | **P0** → P1 | ✅ |
...
| ◑ **N-04** | **Browser verification exists** — `apps/web/e2e/admin-control-center.spec.ts`, 6 passing. ⚠️ **Correction to this row's first version:** two of the four gaps it originally reported — sub-tab deep links discarded, and tab changes not reaching the URL — were **wrong**. Both now pass. Playwright's baseURL was `127.0.0.1`, and Next dev blocks cross-origin dev resources from a bare IP, which kills the HMR socket and with it the client bundle: the page server-renders so the DOM looks right while **nothing hydrates**. Fixed by pointing the harness at `localhost`. The typed-confirmation restore guard is also verified working (wrong string rejected, missing justification rejected). **Genuinely open, both from source rather than the browser:** (a) **legacy routes do not redirect** — no middleware, no `next.config` redirect, `app/admin/users/page.tsx` renders standalone, so `/admin` is a 24th admin surface beside the 23; (b) **the restore guard has no backend** — `backup-restore-panel.tsx` makes 0 API calls and fabricates `"Audit event logged: {…}"`, and no server-side restore endpoint exists. Also fixed in passing: `overview` was missing from the BFF allowlist. RBAC + tenant isolation are covered in the API e2e suite instead | **P1** | ✅ |
