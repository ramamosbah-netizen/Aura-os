...
| ID | Status 2026-08-10 | Sev | Ev |
|---|---|:--:|:--:|
| ◑ **G-20** | **CORE SHIPPED 2026-08-10 with ZERO seeded rules; rule content still blocked on sourcing.** [Discovery](2026-08-10-g20-compliance-discovery.md) · [ADR-0018](../adr/0018-compliance-core.md) · migration `0223` · `modules/compliance` · `/api/v1/compliance/*`. **6 tables** (the gate justified 5; `decisions` is the sixth, because ADR-0018 §7 makes a decision its own append-only entity). One surface for every authority — SIRA and DCD are rows, and the e2e proves adding the second took no code change. 34 domain + 20 HTTP e2e tests. Verified live against Postgres: a rejection survives the later approval, and a renewal issues a new certificate leaving the original's dates intact. RLS **190/190**. **Still open:** the regulatory content itself — no obligation, fee or validity period is seeded, and `requireSource()` makes provenance a type-level requirement so it cannot be added without `source · sourceVersion · retrievedAt · authority`. **UI shipped** — `/compliance`, one screen for every authority with the register, case history (submissions · inspections · decisions · certificates), renewal watch-list and an empty state that explains *why* it is empty; 6 browser e2e. Also open: applicability rules (deferred until a second jurisdiction proves the shape) | **P1** | ✅ |
