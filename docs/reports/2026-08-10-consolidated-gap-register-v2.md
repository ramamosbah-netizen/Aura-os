...
| ID | Status 2026-08-10 | Sev | Ev |
|---|---|:--:|:--:|
| **G-20** | **OPEN — the gate. Discovery done 2026-08-10:** [G-20 Compliance Discovery](2026-08-10-g20-compliance-discovery.md). SIRA/DCD still **0 files**. Discovery found four kernel pieces to reuse rather than rebuild (`DocumentRequirement` + DMS for evidence, `WorkflowService` for the state machine, the expiry projection once consolidated), the six genuinely-absent concepts (authority, case, authority inspection, statutory certificate, company/person licensing, jurisdictional applicability), and a canonical no-fork model — one `Authority` enum, one case model, `scope = project | company | person`. Pre-migration gate answered: **5 new tables, 3 deliberate non-tables**. Also surfaced an N-08 blind spot: the ratchet scans `modules/` only, and `core/src/dms/document-requirement-store.ts` exposes `get(id)` with no tenant | **P1** | ✅ |
