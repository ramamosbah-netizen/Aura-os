...
| ID | Gap | Sev | Ev |
|---|---|:--:|:--:|
| **N-08** | **Application-layer tenant isolation is absent on the CRM read path — isolation rests on RLS alone.** Proven over HTTP with two authenticated tenants (`apps/api/test/rbac-tenant-isolation.e2e-spec.ts`): tenant B **sees** tenant A's account in a list, **fetches** it by id (200), and **mutates** it (200). Cause: `crm-accounts.controller.ts` calls `list({ status, limit: 100 })` with no `tenantId`, and `AccountStore.get(id)` takes no tenant parameter at all. Postgres RLS does hold this line in production (15/15 since G-03) — but that is **one** layer, not two, and it is entirely absent on every no-DB path: the e2e suite, CI, and any dev boot without `DATABASE_URL`. **Before G-03 landed today, neither layer was enforcing**, so cross-tenant reads were reachable in the running deployment. Only the accounts path is measured; the same shape likely repeats across modules and needs a sweep | **P0** | ✅ 🆕 |
