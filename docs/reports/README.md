...
96 reports, newest first within each section. **Index last reviewed: 2026-09-07.**
...
| Date | Report | Status |
|---|---|:--:|
| 2026-09-07 | [Project 360 — Business Logic Conformance](2026-09-07-project-360-business-logic-conformance.md) | 🟢 *29 sections vs domain, BFF, schema and UI — 9 ✅, 12 🟡, 3 ❌* |
...
| Date | Report | Status |
|---|---|:--:|
| 2026-08-31 | [**End-user audit — Work · Communication · Sales & Commercial**](2026-08-31-end-user-audit-work-communication-sales-commercial.md) — UX + business logic + workflow + a prioritized 10-row gap register. Health measured live (typecheck green, 20/20 suite fitness). **Confirmed broken journey (G1): a tender cannot be won through the UI** — both "Won" buttons PATCH `/status`, which the backend rejects (ADR-0021); the governed `award()` endpoint exists in the SDK but no web surface calls it. Also: raw owner IDs shown as names, external comms unconnected, desktop-only tables | 🟢 |
| 2026-08-30 | [**Work · Communication · Sales & Commercial — suite audit**](2026-08-30-work-communication-sales-commercial-suite-audit.md) — structure & completeness of all four suites. Verifies the Signal→Contract→Project chain is genuinely wired via idempotency-annotated reactors; flags the uncommitted Sales IA merge (63 dirty files), the event-level outbox retry invariant, and honest external-provider gaps (M365/WhatsApp unconnected). Static read at `85fdbc70` — **no live journey run** | 🟢 |
