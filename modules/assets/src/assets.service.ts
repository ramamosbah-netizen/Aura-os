...
export class AssetsService {
  ...
  async completeMaintenance(
    ...
    if (actorId) {
      ...
      this.access.assert(actorId, { // ONE VOCABULARY. The route derives `assets.maintenance.complete` and this asserted
        // `assets.maintenance.update` — two names for one act, and the module wildcard that used
        // to be absent from every role meant nobody could reach either.
        permission: 'assets.maintenance.complete', orgPath });
    }
