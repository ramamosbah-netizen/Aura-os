...
export class PostgresCommissioningStore implements CommissioningStore {
  ...
  async saveAsBuiltLink(l: AsBuiltLink): Promise<void> {
    // No ON CONFLICT DO NOTHING: the unique constraint is the guard against linking one drawing to a
    // system twice, and swallowing it would tell the user their link took when it did not.
    await this.pool.query(
      `insert into public.aura_commissioning_asbuilt_links
  ...
  async deleteAsBuiltLink(id: string, tenantId: string): Promise<void> {
    await this.pool.query('delete from public.aura_commissioning_asbuilt_links where id = $1 and tenant_id = $2', [id, tenantId]);
  }
  async listAsBuiltLinks(commissioningId: string, tenantId: string): Promise<AsBuiltLink[]> {
    const res = await this.pool.query(
      'select * from public.aura_commissioning_asbuilt_links where commissioning_id = $1 and tenant_id = $2 order by created_at asc',
      [commissioningId, tenantId],
    );
  ...
  async listAsBuiltLinksForProject(tenantId: string, projectId?: string): Promise<AsBuiltLink[]> {
    const res = await this.pool.query(
      `select * from public.aura_commissioning_asbuilt_links
        where tenant_id = $1 and ($2::text is null or project_id = $2)
        order by created_at asc`,
      [tenantId, projectId ?? null],
    );
...
function toAsBuiltLink(r: Record<string, unknown>): AsBuiltLink {
  return {
    id: r.id as string, tenantId: r.tenant_id as string, companyId: (r.company_id as string) ?? null,
    commissioningId: r.commissioning_id as string, projectId: r.project_id as string,
    documentId: r.document_id as string, linkedBy: (r.linked_by as string) ?? null,
