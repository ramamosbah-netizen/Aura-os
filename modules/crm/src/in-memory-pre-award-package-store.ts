...
export class InMemoryPreAwardPackageStore implements PreAwardPackageStore {
  ...
  private readonly studies = new Map<string, TechnicalStudyRevision>();
  ...
  async saveStudy(s: TechnicalStudyRevision): Promise<void> { this.studies.set(s.id, this.clone(s)); }
  async saveStudyWithClient(_tx: unknown, s: TechnicalStudyRevision): Promise<void> { await this.saveStudy(s); }
  ...
  async getByTender(tenantId: Id, tenderId: Id): Promise<PreAwardPackage | null> {
    for (const p of this.packages.values()) if (p.tenantId === tenantId && p.tenderId === tenderId) return this.clone(p);
    return null;
  }
