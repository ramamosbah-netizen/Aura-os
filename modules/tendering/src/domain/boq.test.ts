...
describe('TenderService BOQ Integration Workflows', () => {
  ...
  const submissionStore = new InMemorySubmissionStore();
  ...
  const service = new TenderService(tenderStore, boqStore, bidScoreStore, estimateStore, submissionStore, mockEvents, mockTx, fakeBus(), mockNumbering, mockAudit);
