// Subcontract form schema — pure metadata shared by the web renderer and the API's
// server-side enforcement (assertFormValid). Moved here from apps/web (gap register
// Vol 23 #8). A factory: the web injects live project options; the API enforces with
// the default empty list (evaluateForm validates values/rules, not option membership).
